...
void debug(DebugPriority priority, DebugType type, String title, double value, String end) {
  if(debugPriority.getValue() == priority && ((type == SETUP && debugSetup.getValue()) || (type == SENSOR && debugSensor.getValue()) || (type == DISPLAYD && debugDisplay.getValue()) || (type == MENUD && debugMenu.getValue()) || (type == TOUCH && debugTouch.getValue())))
