...
const long MHZ14A_RESPONSE_TIME = (unsigned long)60 * 1000;
const unsigned long MHZ19B_RESPONSE_TIME = (unsigned long)5* 1000 -1;
const unsigned long MHZ19C_RESPONSE_TIME = (unsigned long)5* 1000 -1;
...
boolean MHZ::isReady() {
  if (isPreHeating()) {
    ...
    return lastRequest < max(0L, (long) millis() - (long) MHZ14A_RESPONSE_TIME);
  } else if (_type == MHZ19B) {
    return lastRequest < max(0L, (long) millis() - (long) MHZ19B_RESPONSE_TIME);
  } else if (_type == MHZ19C) {
    return lastRequest < max(0L, (long) millis() - (long) MHZ19C_RESPONSE_TIME);
  } else {
    return true;
